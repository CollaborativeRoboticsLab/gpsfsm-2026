...
def save_output_to_file(model_name, prompt_type, task_filename, iteration, result, output_dir="outputs"):
    # Define regular expression pattern to extract the behavior tree from the complete output
    pattern = r"<Plan\b[^>]*>[\s\S]*?</Plan>"
    ...
    if matches:
        ...
        output_dir = output_dir + "/" + model_name + "/" + prompt_type + "/" + base_filename
        os.makedirs(output_dir, exist_ok=True)

        # Construct the filename
        filename = f"{iteration}.xml"
